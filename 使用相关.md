...
调整配置文件，重启服务
```r
为避免权限问题，提前修改/opt/CAPEv2目录下所有文件权限为cape用户所有
cd /opt/CAPEv2 && sudo chown cape:cape . -R

切换到cape用户修改配置文件
sudo su cape

web.conf
把"web_auth"下的"enabled"改为"yes"
...
超级用户需要在这里登陆，之后可以创建其他用户
http://127.0.0.1:8000/admin/
添加用户时，需要设置邮箱，设置后需要在邮箱位置勾选确认邮箱才能登陆，否则会报错提示
