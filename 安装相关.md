...
安装后查看服务，一般 cape-rooter,cape-web 正常就没问题, cape,cape-processor服务可能因为虚机和配置问题不会正常运行  
```r
systemctl status cape*.service
```

如果服务不正常而且用journalctl查看日志有缺少依赖错误
```
...
auxiliary.conf
把"pre_script"的值改为"yes"
把"during_script"的值改为"yes"
...
web.conf
把"pcap_ng"下的"enabled"改为"yes"，可以直接下载解密HTTPS流量的pcapng文件
